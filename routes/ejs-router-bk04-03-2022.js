...
function handleEvent(event) {
    ...
    switch (messageInput){
        ...
        case "Test" : case "ทดสอบ" : case "test" :   
        
        Product.find().exec((err,doc)=>{    
                //console.log(products[i].name)
                //console.log('ตอบกลับข้อความ:', name);
            
         // console.log(products)
        
           var msg = {
                "type": 'text',                
                "text": Object.values(doc).forEach(function(item){ `${item.name} ราคา ${mymodule.formatNumber(item.price)} บาท` })
                 };
                  return client.replyMessage(event.replyToken, msg) ;
                });
